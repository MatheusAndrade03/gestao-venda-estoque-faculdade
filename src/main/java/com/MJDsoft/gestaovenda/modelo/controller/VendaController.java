...
import com.MJDsoft.gestaovenda.modelo.util.AbstractMouseListener;
import com.MJDsoft.gestaovenda.modelo.dao.CategoriaDao;
...
import com.MJDsoft.gestaovenda.modelo.dao.VendaDao;
...
import com.MJDsoft.gestaovenda.modelo.entidades.Venda;
import com.MJDsoft.gestaovenda.modelo.entidades.VendaDetalhes;
import com.MJDsoft.gestaovenda.modelo.exception.NegocioException;
import com.MJDsoft.gestaovenda.modelo.util.VendaRegistroTableModel;
import com.MJDsoft.gestaovenda.modelo.util.VendaTableModel;
...
public class VendaController extends AbstractMouseListener implements ActionListener, KeyListener {
    ...
    public void mouseClicked(MouseEvent me) {
        try {
            ...
            this.vendaDetalhes = this.vendaDao.buscaDetalhesDaVendaPeloId(idVenda);
             this.venda = this.vendaTableModel.getVendas().get(linhaSelecionadaVenda);
        } catch (Exception e) {
        ...
        try {
            ...
            this.nomeDoProduto = (String) this.dashboard.getTabelaVendaRegistro().getModel().getValueAt(linhaSelecionada, 0);
             this.venda = this.vendaTableModel.getVendas().get(linhaSelecionada);
        } catch (Exception e) {
     ...
     private void editar() {
        ...
        if(autenticacaoDao.temPermissao(usuario)){
            if(this.venda != null) {
                ...
                JOptionPane.showMessageDialog(dashboard, "Deves selecionar uma Venda na tabela", "Seleciona um  venda", 0);
            }
        }
    }
    ...
    private void limpaCampo() {
       this.dashboard.getTxtVendaQuantidade().setValue(1);
        ...
        actualizarTabela(vendaDao.todosVendas());
        this.venda = null;
    }
    
     private void actualizarTabela(List<Venda> vendas) {
        this.vendaTableModel = new VendaTableModel(vendas);
        this.dashboard.getTabelaVenda().setModel(vendaTableModel);
        this.dashboard.getLabelHomeVenda().setText(String.format("%d", vendas.size()));
    }
